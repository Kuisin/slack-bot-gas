var slackManagerBot = (function() {
  ...
  const SLACK_BOT_TOKEN = userProperties.getProperty("SLACK_BOT_TOKEN_0001") || null;
  const SLACK_USER_TOKEN = userProperties.getProperty("SLACK_USER_TOKEN_0001") || null;
  const SLACK_VERIFICATION_TOKEN = userProperties.getProperty("SLACK_VERIFICATION_TOKEN_0001");
  ...
  var TOKEN_PATTERNS = {
    ...
    WEBHOOK: /^https:\/\/hooks\.slack\.com\/services\/[A-Z0-9]+\/[A-Z0-9]+\/[a-zA-Z0-9]+$/
  };
  ...
  return {
    ...
    setIgnoreUsers: function(userIds) {
      ...
      const userArray = validUsers.map(function(id) { return id.trim(); });

      try {
        userProperties.setProperty("SLACK_IGNORE_USERS_0001", userArray.join(','));
        
        return {
          ok: true,
          message: "Ignore users set successfully. Total: " + userArray.length + " users.",
          users: userArray
        };
      } catch (error) {
        return {
